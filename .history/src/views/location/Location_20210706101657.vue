  ...
  <div id="orientation">
    <div class="longin">
      ...
      <div v-if="value === ''">
        ...
        <div class="hot" style="margin-left: 15px" @click="Chengdu">
          {{ present }}
        </div>
        <van-divider
        ...
        <div class="city">
          <div class="hot" v-for="item in list" :key="item" @click="hot(item)">
            {{ item }}
          </div>
        </div>
        <van-divider
        ...
        >
          <template v-for="(val, key) in obj">
            ...
            <van-cell
              ...
              :key="item.id"
              @click="alter(item.name)"
            />
          </template>
        </van-index-bar>
      </div>
      <div v-else>
        <div class="hot" v-for="item in arr" :key="item" @click="hot(item)">
          {{ item }}
        </div>
...
export default {
  ...
  methods: {
    ...
    alter(val) {
      this.$store.commit("setCity", val); //修改共享数据
       this.localStorage.setItem('city , val')
      this.$router.push("/");
    },
    // 当前城市
    Chengdu() {
  ...
  mounted() {
    // 当前城市
    this.present = localStorage.getItem("city");
    // 热门城市
