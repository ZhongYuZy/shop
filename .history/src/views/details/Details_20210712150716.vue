<template>
  <div>
    ...
    <van-goods-action>
      ...
      <van-goods-action-icon icon="cart-o" text="购物车" @click="checkLogin('/Shopping')" />
      ...
      >
        ...
        <van-button type="danger" class="button" @click="checkLogin('/Shopping')"
          >立即购买</van-button
        >
      </van-popup>
    </van-goods-action>
...
export default {
  ...
  methods: {
    ...
    addShop(){
      this.$api.addShop(this.goodOne.id).then((res) => {
        console.log(res);
      }).catch((err) => {
        console.log('请求失败' , err);
      })
