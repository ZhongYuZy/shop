<template>
  <div>
    ...
    <div>
      <van-image-preview v-model="show" :image='image' @change="onChange">
        <template v-slot> 第{{ index+1 }}页</template>  
      </van-image-preview>
    </div>
    <div class="middle">
...
export default {
  ...
  methods: {
    onChange(index) {
      (this.show = true), (this.index = index);
    },
    onclick() {
      this.$router.back();
    },
    ...
    goodOne() {
      this.$api
        ...
        .then((res) => {
          ...
          this.image=[this.goodsOne.image_path ,this.goodsOne.image_path]
        })
        .catch((err) => {
